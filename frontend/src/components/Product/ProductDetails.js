...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchHandler = async () => {
      await axios
        .get(`/api/products/${id}`)
        .then((resp) => resp.data)
        .then((data) => setInputs(data.products));
      //error milgaya product ki jagha products ayega maine check kiya ki Products.js me useState ke products use h sayad islye use hua h.
    };
    fetchHandler();
  }, [id]);
  ...
  const sendRequest = async () => {
    await axios
      .put(`http://REACT_APP_BACKEND_ENDPOINT/products/${id}`, {
  ...
  const handleChange = (e) => {
    // to change the data in input field we use AddProduct handleChange() Function
    setInputs((prevState) => ({
      ...prevState,
